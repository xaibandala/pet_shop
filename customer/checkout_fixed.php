...
// Create PayMongo payment link - will be updated by JavaScript to include delivery fee
...
curl_setopt_array($curl, array(
    ...
    CURLOPT_POSTFIELDS => json_encode([
        'data' => [
            'attributes' => [
                ...
                'remarks' => $remarks,
                'redirect' => [
                    'success' => $domain . '/customer/payment_success.php',
                    'failed' => $domain . '/customer/payment_failed.php'
                ]
            ]
        ]
    ]),
    CURLOPT_HTTPHEADER => [
        ...
        "authorization: Basic c2tfdGVzdF81THIxSkNmeVVaY2FyUDNWMkNmY2NBQmk6", // Sandbox test key
...
if ($err) {
    ...
    if ($httpCode === 200 && isset($decoded['data']['attributes']['checkout_url'])) {
        $checkoutUrl = $decoded['data']['attributes']['checkout_url'];
    } else {
        // Fix for PHP 7.0 compatibility - replace ?? with ternary operator
        $errorDetail = isset($decoded['errors'][0]['detail']) ? $decoded['errors'][0]['detail'] : 'Unknown error';
        $paymongoError = "PayMongo API Error: " . $errorDetail;
        $checkoutUrl = '#';
    }
}
?>
                        ...
                        <div class="col-6">
                            ...
                            <div class="col my-3">
                                ...
                                <div class="d-flex w-100 justify-content-between">
                                    ...
                                    <?php if ($checkoutUrl && $checkoutUrl !== '#'): ?>
                                        <a href="<?php echo $checkoutUrl ?>" target="_blank" class="btn btn-success" id="paymongo-btn">Pay with PayMongo</a>
                                    <?php else: ?>
                                        <button type="button" class="btn btn-success" id="paymongo-btn" disabled title="<?php echo htmlspecialchars($paymongoError) ?>">
